...
async function uploadPart(
	...
	if (!response.ok) {
		const text = await response.text();
		throw new Error(
			`Failed to upload part ${partNumber}: ${response.status} ${text}`,
		);
	}
...
async function completeMultipartUpload(
	...
	if (!response.ok) {
		const text = await response.text();
		throw new Error(
			`Failed to complete multipart upload: ${response.status} ${text}`,
		);
	}
}
...
async function uploadPartsInParallel(
	...
	const completedPartNumbers = new Set(
		state.completedParts.map((p) => p.partNumber),
	);
...
export function setupAbortHandler(
	...
	const handler = async () => {
		...
		if (state) {
			console.log(
				`Upload paused at ${state.completedParts.length}/${state.totalParts} parts.`,
			);
...
export async function checkResumableUpload(filePath: string): Promise<{
	...
	if (!state) {
		return { canResume: false, state: null, percentComplete: 0 };
	}

	// Check if file has changed
	if (await hasFileChanged(filePath, state)) {
		return { canResume: false, state, percentComplete: 0 };
	}

	const percentComplete = Math.round(
		(state.completedParts.length / state.totalParts) * 100,
	);

	return { canResume: true, state, percentComplete };
}

/**
 * Abort and clean up an existing upload
 */
export async function cleanupExistingUpload(
	filePath: string,
	config: S3Config,
): Promise<void> {
	const state = await loadState(filePath);
